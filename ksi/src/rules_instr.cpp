...
bool hive::operation_loop_for::parse(state & st, t_tokens & toks, base_log * log) {
	const Char * str;
	//
	{
		state tmp_st = st;
	...
	toks.append( new tokens::token_loop_for_begin(st.liner_.get_pos(st.str_) ) );
	...
	if( last_char == L'\\' ) {
		was_also_block = true;
		toks.append( new tokens::token_loop_for_also_block() );
		if( rules::action_loop::process_block(
			st, toks, log,
			str, lnr, last_char,
			tmp_inf, loop_depth, nest_while_condition, flag_second_part
	...
	toks.append( new tokens::token_loop_for_body(was_also_block) );
	if( rules::action_loop::process_block(
		st, toks, log,
		str, lnr, last_char,
		tmp_inf, loop_depth, nest_loop_body, 0
	...
	if( last_char == L'\\' ) {
		toks.append( new tokens::token_loop_for_body_rest() );
		if( rules::action_loop::process_block(
			st, toks, log,
			str, lnr, last_char,
			tmp_inf, loop_depth, nest_loop_body, flag_second_part
	...
	toks.append( new tokens::token_loop_for_end() );
	st.next_str(str);
	st.liner_ = lnr;
