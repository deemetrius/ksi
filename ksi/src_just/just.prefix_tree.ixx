...
export namespace just {
	...
	struct prefix_tree {
		...
		using t_pass_key = std::conditional_t<s_key_scalar, t_key, const t_key &>;
		using t_key_view = std::conditional_t<s_key_char, std::basic_string_view<t_key>, bool>;
		...
		struct t_node {
			...
			using t_opt_value = std::optional<t_value>;
			...
			pointer find(t_pass_key p_key) {
				...
				return (v_it == m_map.end() ) ? nullptr : (*v_it).second.get();
			}
		...
		prefix_tree(std::initializer_list<t_pair> p_init) requires( s_key_char ) {
			for( const t_pair & v_it : p_init ) {
				typename t_node::pointer v_node = &m_root;
				for( t_pass_key v_key : v_it.m_key ) {
		...
		typename t_node::pointer find(t_pass_key p_key) { return m_root.find(p_key); }
	};
