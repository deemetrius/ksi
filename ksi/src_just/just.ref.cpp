...
template <typename T>
struct closer_deleter {
	using const_pointer = const T *;

	static void close(const_pointer h) { h->deleter_(h); }
};

template <bool Check_null = true, template <typename T1> typename Closer = closer_one>
struct closer_cnt_with {
	template <typename T>
	struct closer {
...
struct closer_cnt_deleter_with {
	template <typename T>
	struct closer {
		using pointer = T *;
		using const_pointer = const T *;
...
template <typename T, template <typename T1> typename Closer = closer_one>
struct with_deleter {
	using t_closer = Closer<T>;
...
template <template <typename T1> typename Closer = closer_one>
struct traits_ref_unique {
	template <typename T>
	struct inner :
		public Closer<T>
	{
		using ref_base = with_handle_mutable<T>;
		using pointer = T *;
